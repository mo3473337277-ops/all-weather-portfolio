...
|---|---|---|---|---|---|
| **V3c 多元** | 简约派 | 8.56% | -5.84% | 1.40 | 6资产逆波动率 60d — 简单干净 |
| **V3-B 风险平价(20d)** | 学院派 | **9.16%** | -7.00% | 1.41 | 5桶等权 RP — 回报最高 |
| **V3-B 保守增强(20d)** | 保守增强 | 7.49% | **-3.83%** | **1.62** | 逆波动率 — 睡得着觉 |

> 全部策略均含 nonferr 趋势过滤 + Gold 抄底(15%/2.5x) + HS300 抄底(35%/2.5x)，月度调仓。
> 资产组合精简为 7 只 ETF：hs300 + us_sp500 + credit + bond_10y + bond_30y + gold + nonferr。
...
**V3-B 风险平价(20d) (学院派)**
- 长期回报最高 CAGR 9.16%，累计 395%；五桶真正等权(20%×5)，全天候理念最纯正
- 回撤最大(-7.00%)，最差年份 -3.97%；桶逻辑比另外两个策略复杂
- 适合：长期持有者(5年+)、认同正统全天候理念、能承受短期波动

**V3-B 保守增强(20d) (保守增强)**
- 回撤最低(-3.83%)，最差年份仅 -0.61%；Sharpe 最高(1.62)，熊市表现最好
- 牛市可能跑输(2019仅+0.44%，2017仅+2.89%)；长期累计回报最低(273%)
...
| 桶 | 资产 | 权重逻辑 |
|---|---|---|
| 增长↑ | hs300, us_sp500 | 桶等权 20%，桶内逆波动率 |
| 收益垫 | credit | 单资产桶，20% |
| 增长↓10Y | bond_10y | 单资产桶，20% |
| 增长↓30Y | bond_30y | 单资产桶，20% |
| 通胀↑ | gold, nonferr | 桶等权 20%，桶内逆波动率 |

10Y/30Y 分拆是关键设计——两者久期差异巨大（~8 vs ~18 年），合并在一个桶里会掩盖真实风险暴露。

### 三条策略的核心差异

| | V3c 多元 | V3-B 风险平价 | V3-B 保守增强 |
|---|---|---|---|
| 权重方法 | 逆波动率 60d | 5桶等权 + 桶内IV | 逆波动率 20d |
| 桶结构 | 无 | 5 桶 × 20% | 无 |
| 资产数 | 6 (无 bond_10y) | 7 | 7 |
...
| 调仓频率 | 月度 | 每月初重算权重 |
| nonferr 趋势过滤 | 60d/75d SMA | 跌破 SMA 则权重转入 credit |
| Gold 抄底 | -15% / 2.5x | 从高点回撤 15% 触发，权重翻 2.5 倍 |
| HS300 抄底 | -35% / 2.5x | 仅史诗级股灾触发 |
