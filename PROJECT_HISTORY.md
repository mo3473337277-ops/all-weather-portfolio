...
**1. 分层 RP 的桶结构是收益天花板。** 4 宏观桶各 25% 的约束人为抬高了低波债券桶的权重，压低了商品和权益的权重。去掉桶层级改用直接逆波动率，保守增强 CAGR 从 5.97% 跳到 6.82%（+0.85%），Sharpe 从 1.83 升至 1.96。
...
**8. 多资产趋势过滤无额外增益。** 对 soymeal 也加趋势过滤不会进一步改善指标。nonferr 是唯一值得过滤的资产——它是回撤的头号元凶。
